...
namespace jaj.Controllers
{
    public class UserController : Controller
    {
        ...
        {
            // Boolean za provjeru da li je user uploadal koju novu sliku profila
            ...
            using (DbBaza dc = new DbBaza())
            {
                ...
                // Ako taj path nije null onda se vade i zapisuju podaci o slici
                if (user2.userInfo != null)
                {
                    ...
                    // Stavlja se writePicture na true zato da se onda izvsi wholeUser.profilePicture = user2.profilePicture;
                    writePicture = true;
                }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var tempProfilePic = wholeUser.profilePicture;
                    wholeUser.Email = user2.Email;
                    wholeUser.Username = user2.Username;
                    wholeUser.FavTag = user2.FavTag;
                    // Ako je neka nova slika uploadana onda se mijenja profilePicture
                    if (writePicture == true)
                    {
                        wholeUser.profilePicture = user2.profilePicture;
                    }
                    ...
                    dc.SaveChanges();
                    if(wholeUser.Username != tempUsername || wholeUser.profilePicture != tempProfilePic)
                    {
                    ...
                    {
                        message = "Editing profile successfully done ";
        ...
        public ActionResult profilePage()
        {
            ...
            using (DbBaza dc = new DbBaza())
            {
                ...
                if (User.Identity.IsAuthenticated)
                {
                    ...
                    user3.UserID = user1.UserID;
                    user3.Username = user1.Username;
                    user3.Email = user1.Email;
                    user3.FavTag = user1.FavTag;
                    user3.profilePicture = user1.profilePicture;

                    var queryVideo = "SELECT videos.videoPath FROM videos WHERE videos.userID = " + user3.UserID;
                    var queryVideoName = "SELECT videos.videoName FROM videos WHERE videos.userID = " + user3.UserID;
                    var queryVideoID = "SELECT videos.videoID FROM videos WHERE videos.userID = " + user3.UserID;
                    using (var context = new DbBaza())
                    {
                        user3.userVideos = context.Database.SqlQuery<string>(queryVideo).ToList();
                        user3.userVideosName = context.Database.SqlQuery<string>(queryVideoName).ToList();
                        user3.userVideosID = context.Database.SqlQuery<string>(queryVideoID).ToList();
                    }
        ...
        [HttpPost]
        public ActionResult PasswordReset(userResetPassword model)
        {
            using (DbBaza dc = new DbBaza())
            {
                    ...
                    var pass = Misc.PasswordHelper.HashPassword(model.Password);
                    user1.Salt = pass.Item1;
                    user1.Password = pass.Item2;

                    dc.Entry(user1).State = EntityState.Modified;
                    dc.Configuration.ValidateOnSaveEnabled = false;
                    dc.SaveChanges();
                    return RedirectToAction("Index", "videoList");
